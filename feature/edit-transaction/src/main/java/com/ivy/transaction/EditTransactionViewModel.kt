...
class EditTransactionViewModel @Inject constructor(
    ...
    private var voiceAgentUiState by mutableStateOf(VoiceAgentUiState.IDLE)
    
    private val audioRecorder by lazy { com.ivy.base.audio.AndroidAudioRecorder(context) }
    private val geminiAgentService by lazy { com.ivy.domain.agent.GeminiAgentService("AQ.Ab8RN6IUNWTzNV4vU6FodeKIyBmH-EsYSLjI434Hq8kEK44G5A") }
    private var voiceAudioFile: java.io.File? = null
    ...
    override fun uiState(): EditTransactionViewState {
        return EditTransactionViewState(
            ...
            voiceAgentUiState = voiceAgentUiState
        )
    }
    ...
    override fun onEvent(event: EditTransactionViewEvent) {
        when (event) {
            ...
            is EditTransactionViewEvent.OnAttachmentChanged -> {
                attachmentUrl = event.path
                saveIfEditMode()
            }
            EditTransactionViewEvent.StartVoiceRecording -> startVoiceRecording()
            EditTransactionViewEvent.StopVoiceRecording -> stopVoiceRecording()
        }
    }
    ...
    private fun findClosestAccount(name: String?): Account? {
        if (name == null) return null
        val cleanName = name.trim().lowercase()
        return accounts.find { it.name.lowercase().contains(cleanName) || cleanName.contains(it.name.lowercase()) }
    }

    private fun findClosestCategory(name: String?): Category? {
        if (name == null) return null
        val cleanName = name.trim().lowercase()
        return categories.find { it.name.value.lowercase().contains(cleanName) || cleanName.contains(it.name.value.lowercase()) }
    }

    private fun startVoiceRecording() {
        try {
            val tempFile = java.io.File(context.cacheDir, "voice_input_temp.mp4")
            if (tempFile.exists()) tempFile.delete()
            voiceAudioFile = tempFile
            audioRecorder.start(tempFile)
            voiceAgentUiState = VoiceAgentUiState.RECORDING
        } catch (e: Exception) {
            e.printStackTrace()
            voiceAgentUiState = VoiceAgentUiState.ERROR
            viewModelScope.launch {
                toaster.show("Error al iniciar grabación")
            }
        }
    }
    ...
    private fun stopVoiceRecording() {
        try {
            audioRecorder.stop()
            voiceAgentUiState = VoiceAgentUiState.PROCESSING
            val file = voiceAudioFile
            if (file == null || !file.exists()) {
                voiceAgentUiState = VoiceAgentUiState.ERROR
                return
            }
            viewModelScope.launch {
                val result = geminiAgentService.processVoiceInput(file)
                result.onSuccess { voiceTx ->
                    amount = voiceTx.amount
                    description = voiceTx.note
                    title = voiceTx.note ?: ""
                    initialTitle = voiceTx.note
                    
                    val mappedType = when (voiceTx.transactionType.uppercase()) {
                    ...
                    val sourceAcc = findClosestAccount(voiceTx.sourceAccountName)
                    if (sourceAcc != null) {
                        account = sourceAcc
                        updateCurrency(sourceAcc)
                    }
                    val destAcc = findClosestAccount(voiceTx.targetAccountName)
                    if (destAcc != null) {
                        toAccount = destAcc
                    }
                    val cat = findClosestCategory(voiceTx.categoryName)
                    if (cat != null) {
                        category = cat
                    }
                    
                    voiceAgentUiState = VoiceAgentUiState.SUCCESS
                    toaster.show("Transacción rellenada por voz")
                    ...
                    it.printStackTrace()
                    voiceAgentUiState = VoiceAgentUiState.ERROR
                    toaster.show("Error del agente de voz: ${it.localizedMessage}")
                }
            }
        } catch (e: Exception) {
            e.printStackTrace()
