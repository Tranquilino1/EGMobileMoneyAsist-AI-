...
fun BoxWithConstraintsScope.EditTransactionScreen(screen: EditTransactionScreen) {
    ...
    UI(
        ...
        voiceAgentUiState = uiState.voiceAgentUiState,
        onStartVoiceRecording = {
            viewModel.onEvent(EditTransactionViewEvent.StartVoiceRecording)
        },
        onStopVoiceRecording = {
            viewModel.onEvent(EditTransactionViewEvent.StopVoiceRecording)
        },
        onDueDateChange = {
...
private fun BoxWithConstraintsScope.UI(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        // Receipt/Comprobante Selection Section
        val launcher = rememberLauncherForActivityResult(
            contract = ActivityResultContracts.GetContent()
        ) { uri ->
            if (uri != null) {
                onAttachmentChange(uri.toString())
            }
        }
        ...
        Column(
            ...
            Text(
                text = "Comprobante de Pago (Recibo)",
                style = UI.typo.nB2.style(
                    color = UI.colors.mediumInverse,
            ...
            if (attachmentUrl != null) {
                Box(
                    modifier = Modifier
                        ...
                        .border(1.dp, UI.colors.gray, RoundedCornerShape(16.dp))
                ) {
                    Image(
                        painter = rememberAsyncImagePainter(attachmentUrl),
                    ...
                    IconButton(
                        ...
                        Icon(
                            painter = androidx.compose.ui.res.painterResource(id = R.drawable.ic_delete),
                ...
                AddPrimaryAttributeButton(
                    icon = R.drawable.ic_custom_camera_m,
                    text = "Añadir comprobante/recibo",
    ...
    // Floating voice assistant button with Glassmorphism
    if (voiceAgentUiState != VoiceAgentUiState.PROCESSING) {
        FloatingActionButton(
            onClick = {
                if (voiceAgentUiState == VoiceAgentUiState.RECORDING) {
                    onStopVoiceRecording()
                } else {
                    onStartVoiceRecording()
                }
            ...
            modifier = Modifier
                ...
                .padding(16.dp)
                .blur(if (voiceAgentUiState == VoiceAgentUiState.RECORDING) 1.dp else 0.dp)
                .border(
                    1.dp,
            ...
            containerColor = if (voiceAgentUiState == VoiceAgentUiState.RECORDING) Color.Red.copy(alpha = 0.8f) else Color.DarkGray.copy(alpha = 0.8f),
            elevation = FloatingActionButtonDefaults.elevation(defaultElevation = 8.dp)
        ) {
            Icon(
                imageVector = if (voiceAgentUiState == VoiceAgentUiState.RECORDING) androidx.compose.material.icons.Icons.Filled.Stop else androidx.compose.material.icons.Icons.Filled.Mic,
                contentDescription = "Agente de Voz",
                tint = Color.White
            )
        }
    } else {
...
private fun BoxWithConstraintsScope.Preview(isDark: Boolean = false) {
    IvyPreview(isDark) {
        UI(
            ...
            onSetTransactionType = {},
            attachmentUrl = null,
            voiceAgentUiState = VoiceAgentUiState.IDLE,
            onAttachmentChange = {}
        )
    }
}
